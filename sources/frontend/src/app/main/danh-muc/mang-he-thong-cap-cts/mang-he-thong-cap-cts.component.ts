...
import { MangCTSDto, MangHeThongCapCTSService, TrangThai, trangThaiOptions } from '@app/proxy';
...
export class MangHeThongCapCTSComponent implements OnInit {
    ...
    constructor(
        ...
        private mangHeThongCapCTSService: MangHeThongCapCTSService,
    ...
    loadData(event?: { sorting?: string }) {
        ...
        this.mangHeThongCapCTSService.getList({
    ...
    editMangCTS(id: number) {
        this.mangHeThongCapCTSService.get(id).subscribe((mangCTS) => {
    ...
    deleteMangCTS(id: number) {
        this.confirmation.warn('::AreYouSureToDelete', '::AreYouSure').subscribe((status) => {
            if (status === Confirmation.Status.confirm) {
                this.mangHeThongCapCTSService.delete(id).subscribe(() => this.loadData());
            }
        });
    }
    ...
    save() {
        ...
        const request = this.selectedMangCTS.id
            ? this.mangHeThongCapCTSService.update(this.selectedMangCTS.id, this.form.value)
            : this.mangHeThongCapCTSService.create(this.form.value);
    ...
    onCheckMaDuplicate(value: string) {
        if (!value || this.selectedMangCTS?.maMangCTS === value) return;
        this.mangHeThongCapCTSService.checkMaDuplicate(value).subscribe({
            next: (isDuplicate) => {
